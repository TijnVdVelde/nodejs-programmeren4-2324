    ...
    it('should retrieve all users successfully', (done) => {
        chai.request(server)
            ...
            .end((err, res) => {
                ...
                expect(res.body).to.have.property('message').that.includes('Found');
    ...
    it('should filter users based on search criteria', (done) => {
        chai.request(server)
            ...
            .end((err, res) => {
                ...
                expect(res.body).to.be.an('object');
                expect(res.body).to.have.property('status', 200);
                expect(res.body).to.have.property('message').that.includes('Found 1 users.');
                expect(res.body.data).to.be.an('array');
    ...
    it('should return an empty list for non-existing search criteria', (done) => {
        chai.request(server)
            ...
            .end((err, res) => {
                ...
                expect(res.body).to.be.an('object');
                expect(res.body).to.have.property('status', 200);
                expect(res.body).to.have.property('message').that.includes('Found 0 users.');
                expect(res.body.data).to.be.an('array');
